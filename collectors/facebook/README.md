...
3. For a listing you want in The Sound Room, export its public listing fields in
   the shape shown by `import.example.json`. Include coordinates so the app can
   enforce the 250-mile radius rather than trusting a city label.
4. Run `FACEBOOK_IMPORT_PATH=/secure/path/listings.json npm run collect`.
